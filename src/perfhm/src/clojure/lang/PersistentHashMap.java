...
final private static Object NOT_FOUND = new Object();
	...
	public String toString() {
		IndentedText it = IndentedText.empty();
		root.describe(it);
		return it.toString();
	}
...
//		Box leafFlag = new Box(null);
	...
	static class IndentedText {
		...
		public IndentedText(StringBuilder sb, int i) {
			char[] ps = new char[i];
			for (int iter = 0; iter < i; ++iter) {
				ps[iter] = ' ';
			}
			this.i 			= i;
			this.prelude 	= new String(ps);
			this.sb 		= sb;
		}

		public IndentedText indent(int ii) {
			return new IndentedText(sb, i + ii);
		}
		...
		public static IndentedText empty() {
			return new IndentedText(new StringBuilder(), 0);
		}
...
final static class ArrayNode implements INode{
	...
	@Override
	public void describe(IndentedText it) {
		it.writeLine(String.format("ArrayNode => count:%d", count));
		IndentedText it2 = it.indent(2);
		IndentedText it3 = it2.indent(2);
		for (int i = 0; i < array.length; ++i) {
			INode node = array[i];
			if (node != null) {
				it2.writeLine(String.format("%d:", i));
				node.describe(it3);
			} else {
				it2.writeLine(String.format("%d: null", i));
			}
		}
	}
...
final static class BitmapIndexedNode implements INode{
	...
	@Override
	public void describe(IndentedText it) {
		it.writeLine(String.format("BitmapIndexedNode => bitmap:0x%x", bitmap));
		IndentedText it2 = it.indent(2);
		IndentedText it3 = it2.indent(2);
		for (int i = 0; i < array.length; i += 2) {
			...
			if (k == null) {
				if (v != null) {
					it2.writeLine(String.format("%d:", i / 2));
					((INode)v).describe(it3);
				} else {
					it2.writeLine(String.format("%d: null", i / 2));
				}
			} else {
				it2.writeLine(String.format("%d: K:%s, V:%s", i / 2, k, v));
			}
		}
	}
...
final static class HashCollisionNode implements INode{
	...
	@Override
	public void describe(IndentedText it) {
		it.writeLine(String.format("HashCollisionNode => hashCollisionKey:%d,count:%d", hash, count));
		IndentedText it2 = it.indent(2);
		for (int i = 0; i < array.length; i += 2) {
			Object k = array[i + 0];
			Object v = array[i + 1];
